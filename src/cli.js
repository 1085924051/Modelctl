...
export async function main(argv = process.argv.slice(2)) {
  ...
  if (command === "pull") { const model = subcommand; if (!model || model.startsWith("--")) throw new Error("pull requires a model id"); const variant = option(rest, "variant") || "english"; const version = option(rest, "version"); const revision = option(rest, "revision"); return print(await post(`${base}/v1/pulls`, { model_id: model, ...(version ? { version } : {}), ...(revision ? { revision } : {}), variant })); }
  if (command === "run") { const model = subcommand; if (!model || model.startsWith("--")) throw new Error("run requires a model id"); const variant = option(rest, "variant") || "english"; const profile = option(rest, "profile") || "auto"; const version = option(rest, "version"); const revision = option(rest, "revision"); return print(await post(`${base}/v1/instances`, { model_id: model, ...(version ? { version } : {}), ...(revision ? { revision } : {}), variant, profile, default: true })); }
...
async function doctor() {
  ...
  const candidates = process.env.MODELCTL_PYTHON ? [process.env.MODELCTL_PYTHON] : (process.platform === "win32" ? ["python"] : ["python3", "python"]);
  let python = null;
